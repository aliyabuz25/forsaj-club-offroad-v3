          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-5xl md:text-8xl font-black italic tracking-tighter uppercase leading-none">
                <span className="text-white">{selectedCategory.name.split(' ')[0]}</span> <span className="text-[#FF4D00]">REYTİNQ</span>
              </h2>
              <p className="text-[#FF4D00] font-black italic text-[11px] md:text-sm mt-2 uppercase tracking-[0.4em]">
                BÜTÜN PİLOTLARIN RƏSMİ SIRALAMASI // 2024
              </p>
            </div>
          </div>
          <button 
          ...
          {selectedCategory.fullStandings.map((driver) => (
            ...
            >
              ...
              <div className="col-span-2 text-right">
                <p className="text-gray-600 font-black italic text-[9px] uppercase tracking-widest mb-1">XAL</p>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-20">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-[#FF4D00] font-black italic text-[11px] md:text-sm mt-2 uppercase tracking-[0.4em]">
              OFFICIAL PILOT STANDINGS // SEASON 2024
            </p>
          </div>
        </div>
      </div>
          ...
          <div key={cat.id} className="flex flex-col gap-10 relative bg-[#111]/40 border border-white/5 p-8 md:p-12 rounded-sm shadow-2xl overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-10 items-stretch relative z-10">
              ...
              <div className="sm:w-3/5 flex flex-col gap-5">
                <p className="text-[#FF4D00] font-black italic text-[10px] uppercase tracking-[0.3em] mb-2">PODIUM LEADERS</p>
                {cat.leaders.map((driver, idx) => (
                  <div 
                    key={driver.id} 
                    className={`relative flex items-center gap-5 p-5 transition-all duration-300 group border rounded-sm overflow-hidden ${
                      driver.rank === 1 
                  ...
                  >
                    {/* Rank Badge */}
                    <div className={`absolute top-0 right-0 px-4 py-1 text-2xl font-black italic skew-x-[-12deg] ${
                      driver.rank === 1 ? 'bg-[#FF4D00] text-black' : 'bg-white/10 text-white/40'
                    }`}>
                      #{driver.rank}
                    </div>

                    <div className={`w-20 h-20 bg-black overflow-hidden shrink-0 border-2 ${
                      driver.rank === 1 ? 'border-[#FF4D00]' : 'border-white/10'
                    }`}>
                    ...
                    <div className="overflow-hidden">
                      <h4 className={`text-xl md:text-2xl font-black italic leading-none uppercase tracking-tighter truncate mb-1 ${
                        driver.rank === 1 ? 'text-white' : 'text-gray-400'
                      }`}>
                        {driver.name}
                      </h4>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent backdrop-blur-[4px] flex flex-col items-center justify-center p-6 text-center">
                   ...
                   >
                     ...
                     <span className="relative z-10 flex items-center justify-center gap-3">
                       HAMISINA BAX <ArrowRight size={18} className="group-hover/btn:translate-x-2 transition-transform" />
                     </span>
                   </button>
                </div>
              </div>
