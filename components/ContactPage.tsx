...
import { MapPin, Phone, Mail, Instagram, Youtube, Facebook, Send, Info, ChevronDown, Clock, Map as MapIcon, Music2 as TikTok } from 'lucide-react';
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-12">
          ...
          <div className="lg:w-[40%] bg-[#111] border border-white/5 p-10 flex flex-col justify-between relative shadow-2xl rounded-sm">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter mb-2 leading-none text-white">
                {settings.address || 'AZADLIQ 102, BAKI'}
              </h3>
            ...
            <div className="mt-16 space-y-6">
              <div className="flex items-center gap-5 group">
                ...
                <div>
                  ...
                  <p className="text-2xl font-black italic uppercase tracking-tighter text-white">{settings.phone || '+994 50 000 00 00'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-5 group">
                ...
                <div>
                  ...
                  <p className="text-xl font-black italic uppercase tracking-tighter text-white">{settings.email?.toUpperCase() || 'INFO@FORSAJ.AZ'}</p>
                </div>
              </div>

              <div className="flex gap-2 pt-6">
                {socialLinks.map((link, i) => (
                  <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="flex-1 bg-white/5 py-4 flex justify-center text-gray-500 hover:text-black hover:bg-[#FF4D00] transition-all shadow-lg border border-white/5">
                    <link.Icon size={20} />
                  </a>
                ))}
              </div>
          ...
          <div className="lg:w-[60%] flex flex-col gap-6">
            <DepartmentCard
              ...
              email={settings.email?.toUpperCase() || "HQ@FORSAJ.AZ"}
              icon={<Send size={24} />}
            />
            <DepartmentCard
              title="MEDİA VƏ PR"
              desc="MƏTBUAT VƏ ƏMƏKDAŞLIQ"
              email={`PR@${settings.email?.split('@')[1] || 'FORSAJ.AZ'}`}
              icon={<Youtube size={24} />}
            />
            <DepartmentCard
              title="TEXNİKİ DƏSTƏK"
              desc="PİLOTLAR ÜÇÜN TEXNİKİ YARDIM"
              email={`TECH@${settings.email?.split('@')[1] || 'FORSAJ.AZ'}`}
              icon={<Info size={24} />}
            />
          </div>
        </div>
          ...
          <form className="space-y-10" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-3">
                <label className="text-gray-600 font-black italic text-[10px] uppercase tracking-[0.3em]">{t('contact.form_name')}</label>
                <input type="text" placeholder={t('contact.form_name')} className="w-full bg-[#111] border border-white/5 text-white p-5 font-black italic text-xs uppercase outline-none focus:border-[#FF4D00] transition-colors placeholder:text-gray-800" />
              </div>
              <div className="space-y-3">
                <label className="text-gray-600 font-black italic text-[10px] uppercase tracking-[0.3em]">{t('contact.form_email')}</label>
                <input type="text" placeholder={t('contact.form_email')} className="w-full bg-[#111] border border-white/5 text-white p-5 font-black italic text-xs uppercase outline-none focus:border-[#FF4D00] transition-colors placeholder:text-gray-800" />
              </div>
            </div>

            <div className="space-y-3">
              <label className="text-gray-600 font-black italic text-[10px] uppercase tracking-[0.3em]">{t('footer.stay_updated')}</label>
            ...
            <div className="space-y-3">
              <label className="text-gray-600 font-black italic text-[10px] uppercase tracking-[0.3em]">{t('contact.form_message')}</label>
              <textarea placeholder={t('contact.form_message')} rows={5} className="w-full bg-[#111] border border-white/5 text-white p-5 font-black italic text-xs uppercase outline-none focus:border-[#FF4D00] transition-colors resize-none placeholder:text-gray-800" />
            </div>
            ...
            <button className="w-full bg-[#FF4D00] text-black py-6 font-black italic text-3xl uppercase tracking-tighter flex items-center justify-center gap-4 hover:bg-white transition-all transform shadow-[0_15px_40px_rgba(255,77,0,0.2)]">
              {t('contact.send_button')} <Send size={28} />
            </button>
          </form>
        </div>
      </div>
