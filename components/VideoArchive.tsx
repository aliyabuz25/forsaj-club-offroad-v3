...
const VideoArchive: React.FC<VideoArchiveProps> = ({ onViewChange }) => {
  ...
  // Merge videos from video_archive.json and gallery sections
  const allVideos = [
    ...(videoArchive || []),
    ...gallery.flatMap(section => section.videos.map(v => ({
      ...v,
      ...
      <div className="flex justify-between items-end mb-16 px-2">
        ...
        >
          <span className="transform skew-x-12 flex items-center gap-2 uppercase tracking-widest">
            <TranslatableText text={t('common.all', 'HAMISI')} /> <ArrowRight className="w-5 h-5" />
          </span>
        </button>
      </div>
          ...
          >
            ...
            <div className="p-6 border-t border-white/5 flex-grow">
              <TranslatableText
                text={video.title}
                as="h3"
                className="text-xl font-black italic text-white uppercase leading-tight group-hover:text-[#FF4D00] transition-colors tracking-tighter"
              />
            </div>
          </div>
        ))}
